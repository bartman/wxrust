...
- Bulk fetch command to download workouts from server into cache, with options for diff, force, file import, and `--stats` (prints per-date skip messages and `X workouts, Y seconds, x.y T/s, x.y MB/s` after the progress line). Fetch skips already-cached dates by checking file existence (no parse); skip messages (`YYYY-MM-DD already cached, skipping`) print only with `--stats`. Network fetches use batched concurrent `jday` queries. HTTP response body sizes are counted in `api::transfer_stats`.

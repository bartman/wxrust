...
pub async fn login<C: crate::api::ApiClient>(
    client: &C,
    ...
    let credentials = fs::read_to_string(credentials_path).map_err(|_| {
        format!(
            "{} not found. Please create it with email on first line and password on second.",
            credentials_path
        )
    })?;
    ...
    if lines.len() < 2 {
        return Err(format!(
            "{} must have at least 2 lines: email and password",
            credentials_path
        ));
    }
    ...
    let request = models::GraphQLRequest {
        ...
        variables: models::LoginVariables {
            u: email,
            p: password,
        },
    };

    let response = api::login_request(client, &request)
        .await
...
pub fn load_uid_from_cache(token_path: &str) -> Result<u32, String> {
    let contents = fs::read_to_string(token_path)
        .map_err(|_| format!("Token file {} not found", token_path))?;
    let cached: CachedToken =
        serde_json::from_str(&contents).map_err(|_| "Invalid token cache format".to_string())?;
    let now = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap()
        .as_secs();
